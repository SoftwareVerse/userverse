...
async def test_a_get_company_one_by_id_success(client, login_token, seed_companies):
    ...
    response = await client.get(f"/company?company_id={company_id}", headers=headers)
    #
...
async def test_b_get_company_one_by_email_success(
    client, login_token, test_company_data, seed_companies
):
    """Test creating Company One using User One's token"""
    headers = {"Authorization": f"Bearer {login_token}"}
    company_email = test_company_data["company_one"]["email"]
    response = await client.get(f"/company?email={company_email}", headers=headers)
    assert response.status_code in [200, 201]
    #
    json_data = response.json()
    assert "message" in json_data
...
async def test_c_get_company_invalid_args(client, login_token):
    """Test creating Company One using invalid args"""
    headers = {"Authorization": f"Bearer {login_token}"}
    response = await client.get("/company", headers=headers)
...
async def test_d_get_company_not_found(client, login_token, seed_companies):
    """Test creating Company One using User One's token"""
    headers = {"Authorization": f"Bearer {login_token}"}

    response = await client.get(f"/company?company_id={uuid4()}", headers=headers)
...
async def test_e_get_company_without_associated(client, login_token, seed_companies):
