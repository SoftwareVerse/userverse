...
async def test_verify_user_account_activates_user(client):
    ...
    try:
        ...
        response = await client.get(f"/user/verify?token={_verification_token(email)}")
...
async def test_verify_user_account_is_idempotent_for_active_user(
...
async def test_verify_user_account_rejects_wrong_token_type(
...
async def test_resend_verification_email_skips_verified_user(
...
async def test_resend_verification_email_rate_limited(client):
...
async def test_resend_verification_email_short_circuits_when_verification_disabled(
    client, monkeypatch
):
    sent_messages = []
