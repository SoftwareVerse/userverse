...
def test_verify_user_account_activates_user(client):
    ...
    assert response.json()["message"] == UserResponseMessages.USER_ACCOUNT_VERIFIED.value
...
def test_resend_verification_email_requires_authenticated_user(
    client, login_token, test_user_data
):
